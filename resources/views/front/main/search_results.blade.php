...
@section('home_2')
    ...
    <section class="properties-list featured portfolio blog">
        <div class="container">
                    ...
                    <div class="detail-wrapper-body">
                        <div class="listing-title-bar">
                            <div class="text-heading text-left">
                                <p><a href="{{ route('index') }}">Trang chủ </a> &nbsp;/&nbsp; <span>danh sách đề xuất</span>
                                </p>
                            </div>
                            <h3>
                                Kết quả tìm kiếm
                            ...
                            <div class="text-heading text-left">
                                <p class="font-weight-bold mb-0 mt-3">
                                    <label
                                        ...
                                        for="inputGroupSelect01"><i class="fas fa-align-left fs-16 pr-2"></i>Kết quả tìm
                                        kiếm: Có {{ $posts->total() }} tin đăng cho thuê
                                    </label>
                                </p>
                            </div>
                        </div>
            ...
            <!-- Block heading end -->
            <div class="row featured portfolio-items" id="post-list-sort">
                @include('front.main.sort_page.posts_list_sort', ['posts' => $posts])
            </div>
...
@section('customJs')
    ...
    <script>
        $(document).ready(function() {
            // Lấy tham số tìm kiếm từ form và request
            function getSearchParams() {
                return {
                    keyword: $('input[name="keyword"]').val() || '{{ request('keyword') }}',
                    category: $('select[name="category"]').val() || '{{ request('category') }}',
                    provinceName: $('#provinceName').val() || '{{ request('provinceName') }}',
            ...
            function loadPosts(sort, page) {
                ...
                $.ajax({
                    url: '{{ route('posts.search.sort') }}',
                    type: 'GET',
                    data: params,
                    success: function(response) {
                        $('#post-list-sort').html(response.posts_html);
