...
<style>
    ...
    .input-error {
        ...
        background-color: #ffe6e6; /* Nhẹ nhàng báo lỗi */
    }
                        ...
                        <div id="tab-2" class="tab-contents">
                            <div class="custom-form">
                                <form name="registerform" class="main-register-form" id="registrationForm">
                                    ...
                                    <div>
                                        <input type="radio" name="account_type" value="user" {{ old('account_type') == 'user' ? 'checked' : '' }}> 
                                        <span style="margin-right: 5px">Tìm kiếm</span>
                                        <input type="radio" name="account_type" value="poster" {{ old('account_type') == 'poster' ? 'checked' : '' }}> 
                                        <span>Chính chủ</span>
                                    </div>
                                    <span id="account_type_error" class="error-message"></span>
...
<script>
    ...
    $.ajax({
        ...
        success: function (response) {
            // Xóa lỗi cũ trước khi hiển thị lỗi mới
            $(".error-message").html(""); // Xóa nội dung lỗi
            $("input").removeClass("input-error"); // Xóa viền đỏ trên input

            if (response.status === false) {
                $.each(response.errors, function (field, messages) {
                    let errorField = $("#" + field + "_error"); // Thẻ <span> hiển thị lỗi
                    if (errorField.length) {
                        errorField.html(messages[0]); // Lấy lỗi đầu tiên hiển thị
                    }
                    $("#" + field).addClass("input-error"); // Tô viền đỏ input có lỗi
                });
            } else {
